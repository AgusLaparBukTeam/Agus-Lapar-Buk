...
def upgrade() -> None:
    ...
    if "document_reference" not in columns:
        op.add_column("shipment_documents", sa.Column("document_reference", sa.String(160), nullable=True))
    ...
    if "ix_shipment_documents_document_reference" not in indexes:
        op.create_index("ix_shipment_documents_document_reference", "shipment_documents", ["document_reference"])
...
def downgrade() -> None:
    ...
    columns = {column["name"] for column in sa.inspect(op.get_bind()).get_columns("shipment_documents")}
