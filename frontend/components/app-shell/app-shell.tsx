...
function activeLabel(pathname: string) { for (const group of groups) { const match = group.items.find(([href]) => pathname === href || pathname.startsWith(`${href}/`)); if (match) return match[1]; } return "Overview"; }
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="console-shell" data-sidebar-collapsed={collapsed}>
    ...
    {searchOpen && <div className="command-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSearchOpen(false); }}><div className="command-dialog" role="dialog" aria-modal="true" aria-labelledby="command-title"><div className="command-dialog__header"><MagnifyingGlass size={18} /><input autoFocus value={search} onChange={(event) => { setSearch(event.target.value); setSelectedIndex(0); }} placeholder="Search GateGuard" aria-label="Search GateGuard" /><Button variant="ghost" shape="square" size="sm" icon={X} aria-label="Close search" onClick={() => setSearchOpen(false)} /></div><div className="command-dialog__hint" id="command-title">Search shipments, evidence, people, and pages <span>{resultCount} results · Esc to close</span></div><div className="command-dialog__results">{navResults.map((item, index) => <button type="button" key={item.href} className={`command-result ${selectedIndex === index ? "is-selected" : ""}`} onMouseEnter={() => setSelectedIndex(index)} onClick={() => { setSearchOpen(false); router.push(item.href); }}><item.Icon size={17} /><span><strong>{item.label}</strong><small>{item.group} · {item.keywords.split(" ").slice(0, 3).join(" · ")}</small></span><CaretRight size={14} /></button>)}{remoteResults.map((item, index) => <button type="button" key={`${item.type}-${item.id}`} className={`command-result ${selectedIndex === navResults.length + index ? "is-selected" : ""}`} onMouseEnter={() => setSelectedIndex(navResults.length + index)} onClick={() => { setSearchOpen(false); router.push(item.href); }}><MagnifyingGlass size={17} /><span><strong>{item.label}</strong><small>{item.type} · {item.description}</small></span><CaretRight size={14} /></button>)}{!resultCount && <div className="command-empty">No matching records. Try a shipment reference, party, document, or exception.</div>}</div></div></div>}
  </div>;
}
