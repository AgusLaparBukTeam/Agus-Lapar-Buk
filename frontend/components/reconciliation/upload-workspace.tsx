...
export function UploadWorkspace() {
  ...
  return <main className="document-check-page"><div className="document-check-intro"><h1>{t("checkShipmentDocuments")}</h1><p>{t("checkShipmentDescription")}</p></div><div className="grid gap-3 md:grid-cols-3"><UploadSlot label={t("deliveryOrder")} hint={t("fileTypes")} file={files.delivery_order} onFile={(file, error) => setFile("delivery_order", file, error)} /><UploadSlot label="Invoice" hint={t("fileTypes")} file={files.invoice} onFile={(file, error) => setFile("invoice", file, error)} /><UploadSlot label={t("packingList")} hint={t("fileTypes")} file={files.packing_list} onFile={(file, error) => setFile("packing_list", file, error)} /></div><section className="mt-4 flex flex-col items-start justify-between gap-3 rounded-md border border-kumo-line bg-kumo-base p-3 sm:flex-row sm:items-center"><div><div className="text-sm font-medium text-kumo-contrast">{ready ? t("documentsReady") : t("addAllDocuments")}</div><div className="mt-0.5 text-sm text-kumo-neutral-750">{t("uploadCompareReview")}</div></div><Button variant="primary" disabled={!ready || mutation.isPending} onClick={() => mutation.mutate()} className="min-w-44">{mutation.isPending ? <><SpinnerGap size={15} className="animate-spin" /> {t("checkingDocuments")}</> : t("checkDocuments")}</Button></section>{mutation.isPending && <div className="mt-3 rounded-md border border-kumo-info bg-kumo-info-tint p-3 text-sm text-kumo-info" role="status">{t("readingDocuments")}</div>}</main>;
}
