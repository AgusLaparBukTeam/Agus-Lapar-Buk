...
export default function ShipmentDetailPage() {
  ...
  const renderTable = (rows: Row[], columns: Array<[string, string]>) => rows.length ? <div className="table-scroll"><Table><Table.Header sticky><Table.Row>{columns.map(([key, heading]) => <Table.Head key={key}>{heading}</Table.Head>)}</Table.Row></Table.Header><Table.Body>{rows.map((row, index) => <Table.Row key={String(row.id || index)}>{columns.map(([key]) => <Table.Cell key={key}>{value(row, key)}</Table.Cell>)}</Table.Row>)}</Table.Body></Table></div> : <div className="empty-state"><strong>No records yet</strong><span>Information will appear as this shipment is prepared.</span></div>;
  return <div className="operations-page"><Link className="back-link" href="/shipments"><ArrowLeft size={15} /> Back to shipments</Link><PageHeader icon={Package} title={value(item, "internal_reference")} description={`${value(item, "origin")} to ${value(item, "destination")}`} actions={<State status={value(item, "status")} />} /><Tabs tabs={tabs.map((itemTab) => ({ value: itemTab, label: itemTab }))} value={tab} onValueChange={(value) => setTab(value as Tab)} className="detail-tabs" aria-label="Shipment sections" />
    ...
    <section className="data-panel data-panel--wide release-panel"><div className="data-panel__header"><div><h2>Decision and movement</h2><p>Reviewer actions are recorded as immutable events.</p></div></div><AppTextarea label="Decision note" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Record the evidence or review that supports this decision" minLength={5} description="At least 5 characters. This note is included in the immutable decision record." />{decision.isError && <p role="alert" className="form-error">{(decision.error as Error).message}</p>}{move.isError && <p role="alert" className="form-error">{(move.error as Error).message}</p>}<div className="form-panel__actions">{canDecide && <><Button variant="secondary" disabled={decision.isPending || reason.trim().length < 5} onClick={() => decision.mutate("HOLD")}>Place on hold</Button><Button disabled={decision.isPending || reason.trim().length < 5 || Number(item.open_tasks || 0) > 0} onClick={() => decision.mutate("AUTHORIZE")}>Authorize release</Button></>}{item.status === "RELEASE_AUTHORIZED" && canDecide && <Button onClick={() => move.mutate("DISPATCHED")}>Mark dispatched</Button>}{item.status === "DISPATCHED" && canDecide && <Button onClick={() => move.mutate("CLOSED")}>Close shipment</Button>}{!canDecide && <span className="muted-label">A reviewer or administrator can record movement decisions.</span>}</div></section>
  </div>;
}
