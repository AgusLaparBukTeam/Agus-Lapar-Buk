...
export default function ShipmentsPage() {
  ...
  return <div>
    ...
    <section className="filter-bar" aria-label="Shipment filters"><label className="filter-search"><MagnifyingGlass size={17} /><input value={query} onChange={(event) => { setPage(1); setQuery(event.target.value); }} placeholder="Search by shipment reference or destination" aria-label="Search shipments" /></label><AppSelect ariaLabel="Filter shipments by status" value={status} onValueChange={(nextStatus) => { setPage(1); setStatus(nextStatus); }} options={[{ value: "", label: "All statuses" }, ...Object.entries(statusLabels).map(([value, label]) => ({ value, label }))]} /></section>
